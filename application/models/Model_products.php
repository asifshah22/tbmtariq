...
public function getAllProductRangeData($range_to = null, $range_from = null)
{
    // Convert the provided date range to the YYYY-MM-DD format
    $range_to_converted = date('Y-m-d', strtotime(str_replace('/', '-', $range_to)));
    $range_from_converted = date('Y-m-d', strtotime(str_replace('/', '-', $range_from)));

    // Add condition for date range if provided
    $where_clause = '';
    $params = array();
    if ($range_from_converted !== null && $range_to_converted !== null) {
        $where_clause = " WHERE DATE(date_time) BETWEEN ? AND ?";
        $params = array($range_from_converted, $range_to_converted);
    }
...
public function getStockWithUnitsProductsData($date_time = null)
{
    // Initialize the WHERE clause for filtering
    $where_condition = '';

    // Check if $date_time parameter is provided
	if ($date_time !== null && $date_time !== '') {
        // Add date_time condition to the WHERE clause
        $where_condition = " WHERE DATE(products.date_time) = ?";
    }

	//$where_condition .= " AND items_stock.is_deleted = 0"; // Assuming is_delete column exists in products table
	if ($date_time === null || $date_time === '') {
        $where_condition .= " AND items_stock.is_deleted = 0"; // Assuming is_delete column exists in products table
    }

    // Construct the SQL query
    $sql = "SELECT COUNT(*), SUM(items_stock.quantity) as sum_qty, items_stock.unit_id, items_stock.product_id, items_stock.category_id, items_stock.id, items_stock.is_deleted, products.date_time
            FROM items_stock
            INNER JOIN products ON items_stock.product_id = products.id" . $where_condition . "
            GROUP BY items_stock.unit_id, items_stock.product_id, items_stock.category_id, items_stock.id
            ORDER BY items_stock.id DESC";
...
public function getStockWithUnitsProductsReportsData($date_time = null)
{
    // Initialize the WHERE clause for filtering
    $where_condition = '';
    ...
    /* public function getStockWithUnitsDataremove($id)
    {
        if($id) {
            $array = array('id' => $id);
            $this->db->where($array);
            //$delete = $this->db->delete('items_stock');

            header('Location: https://tbmtariq.tbmengineering.com/index.php/Product/manage_stock');
            //return ($delete == true) ? true : false;
        }
    } */
	...
	public function getStockWithUnitsDataRemove($id)
{
    if($id) {
        $array = array('id' => $id);
        $update_data = array('is_deleted' => 1); // Update the 'is_deleted' column to 1
        $this->db->where($array);
        $this->db->update('items_stock', $update_data); // Update the row with the specified id
        return true; // Assuming successful update
    }
    return false; // No id provided or other error
}
